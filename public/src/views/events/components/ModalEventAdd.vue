    ...
    <div class="modal-dialog" role="document">
      <div class="modal-content">
          ...
          <div class="card">
            <div class="card-body">
              ...
              <div class="mb-3">
                ...
                <textarea v-model="form.package" class="form-control form-control-lg"></textarea>
              </div>
              <div class="row">
                <div class="col-sm-12 col-md-6">
                  <div class="mb-3">
                    <label class="form-label">Min. Occupancy</label>
                    <input v-model="form.min_occupancy" type="number" class="form-control form-control-lg">
                  </div>
                </div>
                <div class="col-sm-12 col-md-6">
                  <div class="mb-3">
                    <label class="form-label">Max. Occupancy</label>
                    <input v-model="form.max_occupancy" type="number" class="form-control form-control-lg">
                  </div>
                </div>
              </div>
              <div class="mb-3">
        ...
        <div class="modal-footer">
          ...
          <button class="btn btn-primary" @click="saveEvent()" :disabled="loading">Save</button>
        </div>
      </div>
    </div>
  </div>
...
<script lang="ts">
  ...
  export default defineComponent({
    ...
    methods: {
      ...
      async saveEvent() {
        this.loading = true;
        await axios.get( variable()['api_main'] + "events/add?" + $.param(this.form)).then( async (response) => {
          this.loading = false;
          if(response.data?.success) {
            Swal.fire({
              ...
              this.form.package         = '';
              this.form.min_occupancy   = 0;
              this.form.max_occupancy   = 0;
              this.form.price           = 0;
              this.$emit('refresh');
            });
          }
